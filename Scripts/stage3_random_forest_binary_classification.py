...
"""
Stage 3 (Sebastian Project) — Random Forest for Binary Classification
...
- Class imbalance handling:
    RandomForestClassifier uses class_weight="balanced" to reduce bias toward the majority class.
...
RUN_FOLDER = Path("Step3_RF_Binary")
...
def setup_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("stage3_rf_binary")
...
def plot_feature_importance(importances: np.ndarray, feature_names: List[str], out_path: Path, top_n: int = 25) -> pd.DataFrame:
...
def main() -> None:
    ...
    logger.info("=== Stage 3: Random Forest — BINARY classification (authentic vs adulterated) ===")
    ...
    # Sample weights (balanced); used consistently in CV and final fit
    ...
    # Defensible hyperparameter space
    param_distributions = {
    ...
    roc_auc = plot_roc(y_test, y_score, folders["figures"] / "ROC_RF_binary_external_test.png")
    ap = plot_pr(y_test, y_score, folders["figures"] / "PR_RF_binary_external_test.png")
    ...
    plot_confusion_matrix(
        cm,
        labels=label_names,
        title="Confusion Matrix — Random Forest (Binary, External Test, Acq 6)",
        out_path=folders["figures"] / "confusion_matrix_RF_binary_external_test.png",
    )
    ...
    df_imp = plot_feature_importance(
        ...
        out_path=folders["figures"] / "feature_importance_RF_top25.png",
        top_n=25,
    )
    ...
    # Save to Excel
    out_xlsx = folders["outputs"] / "Stage3_RandomForest_Binary_Results.xlsx"
    ...
    out_json = folders["outputs"] / "Stage3_RandomForest_Binary_Summary.json"
