...
"""
Stage 3 (Sebastian Project) — Random Forest Regressor for Regression
...
RUN_FOLDER = Path("Step3_RF_Regression")
...
def setup_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("stage3_rf_regression")
...
def plot_predicted_vs_observed(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path) -> None:
    ...
    plt.title("Random Forest Regression — Predicted vs Observed (External Test, Acq 6)")
    plt.tight_layout()
...
def plot_residuals(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path) -> None:
    ...
    plt.title("Random Forest Regression — Residuals (External Test, Acq 6)")
    plt.tight_layout()
    plt.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def main() -> None:
    ...
    logger.info("=== Stage 3: Random Forest — Regression (concentration) ===")
    ...
    # Defensible hyperparameter space (laptop-friendly)
    param_distributions = {
    ...
    plot_predicted_vs_observed(
        y_true=y_test,
        y_pred=y_pred_test,
        out_path=folders["figures"] / "predicted_vs_observed_RF_external_test.png",
    )
    plot_residuals(
        y_true=y_test,
        y_pred=y_pred_test,
        out_path=folders["figures"] / "residuals_RF_external_test.png",
    )
    ...
    # Save to Excel
    out_xlsx = folders["outputs"] / "Stage3_RandomForest_Regression_Results.xlsx"
    ...
    out_json = folders["outputs"] / "Stage3_RF_Regression_Summary.json"
